...
export class WithdrawDetailPage extends SecondLevelPage {
    ...
    promptLimit: any = {
        title1:
            window["language"]["NO_WITHDRAWAL_AMOUNT"] || "没有提现金额限制",
        title2: "",
    };
    promptAmount: any = {
        min: undefined,
    ...
    openWithdrawAddressSelector() {
        ...
        const selector = this.modalCtrl.create(WithdrawAddressListPage, {
            title:
                window["language"]["WITHDRAWAL_ADDRESS_MANAGEMENT"] ||
                "提现地址管理",
            productInfo,
            selected_data:
    ...
    cancelWithdrawModal(transactionId?: string, id?: number) {
        let modal = this.alertCtrl.create({
            ...
            message:
                window["language"]["WITHDRAWAL_ORDER"] ||
                "确定取消这条提现订单吗？",
            buttons: [
                {
            ...
            .then(() => {
                return this.accountService
                    ...
                    .then(data => {
                        ...
                        this.access_info.balance = new BigNumber(
                            "" + this.access_info.balance || "0",
                        ).toString();
                    });
            })
            .catch(err => {
                ...
                .then(data => {
                    ...
                    if (limitedQuota) {
                        if (!limitedQuota["min"] && !limitedQuota["max"]) {
                            this.promptLimit.title1 =
                                window["language"]["NO_WITHDRAWAL_AMOUNT"] ||
                                "没有提现金额限制";
    ...
    async submitWithdrawAppply() {
        ...
        if (!(this.personalDataService.certifiedStatus == "2")) {
            return Promise.reject(
                new Error(
                    `${window["language"]["VERIFICATION"] || "实名认证"}：${this
                        .personalDataService.realname ||
    ...
    getTransactionLogs() {
        ...
        return this._getWithdrawLogs().then(data => {
    ...
    private async _formatWithdrawLogs(transaction_logs: TransactionModel[]) {
        ...
        const formated_transaction_logs = await Promise.all(
            transaction_logs.map(async transaction => {
                if (transaction.productHouseId)
                    productHouseId = transaction.productHouseId;
    ...
    async validateIdentify() {
        ...
        if (
            ...
            alert["title"] = `${window["language"][
                "REAL_NAME_AUTHENTICATION"
            ] || "实名认证"}`;
            alert["subTitle"] = `${this.personalDataService.realname ||
                this.personalDataService.certifiedMsg}`;
            alert["buttons"] = [
                {
                    text: window["language"]["CANCEL"] || "取消",
