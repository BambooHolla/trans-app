...
@Component({
    selector: "page-fund-statement",
    templateUrl: "fund-statement.html",
})
export class FundStatementPage {
    piedata: any;
    pieTitle: any;
    piedata1: any;
    pieTitle1: any;
    ...
    titleArray: string[] = ["今天", "本周", "本月", "近三个月", "全部"];
    distancelinedataList: any[] = [];
    secondtitleArray: string[] = ["总资产", "累计盈亏"];
    seconddistancelinedataList: any[] = [];

    initData() {
        const bardata = [];
        const xv = ["11", "10", "9", "8", "7"];
        const yv = [15000, 4000, 8000, 2000, 6000];
        this.barTitle = "每日盈亏";
        bardata.push(xv, yv);
        ...
        var voldata = splitData(data);
        var distancelinedata = [];
        for (var i = 0; i < voldata.vols.length; i++) {
            distancelinedata.push([
                voldata.times[i],
                voldata.vols[i],
                voldata.difs[i],
            ]);
        ...
        for (let j = 0; j < this.titleArray.length; j++) {
            this.distancelinedataList.push(distancelinedata);
        }

        for (let j = 0; j < this.secondtitleArray.length; j++) {
            this.seconddistancelinedataList.push(distancelinedata);
        }

        var name = ["云代码", "长虹鞋业", "斯为通"];
        var value = ["36.88", "20", "15"];
        this.pieTitle = "盈利Top3";
        const piedata = new Array();
        for (var a = 0; a < name.length; a++) {
            piedata[a] = [name[a], value[a]];
        }
        this.piedata = piedata;

        var name1 = ["云代码", "长虹鞋业", "斯为通"];
