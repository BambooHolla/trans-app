...
export class TradeInterfacePage {
    ...
    changeByStep(
        ...
        let length = 0;
        if (isNaN(step)) {
            length = this[target].split(".")[1]
                ? this[target].split(".")[1].length
                : length;
        ...
        const result = Math.max(
            0,
            Math.round(+this[target] * invBase + step * invBase) / invBase,
        );
        //强制刷新数据hack处理
    ...
    checkMax(price = this.price) {
        ...
        if (this._tradeType$.getValue() === 1) {
            ...
            const target = personalStockList.filter(
                ({ stockCode }) => stockCode === traders[0],
            );
            let saleableQuantity =
                (target && target.length != 0
                    ? target
                    : [{ saleableQuantity: 0 }])[0].saleableQuantity / 1;
            this.maxAmount = Number(price)
                ? saleableQuantity / Number(price)
                : "--";
        } else if (this._tradeType$.getValue() === 0) {
            //最大持仓
            const target = personalStockList.filter(
                ({ stockCode }) => stockCode === traders[1],
            );
            console.log(target);
            this.maxAmount =
                (target && target.length != 0
    ...
    async getFee(rate) {
        ...
        const rateTarget =
            rate.targetType === "001"
                ? await this.stockDataService.getProduct(traders[1])
                : rate.targetType === "002"
                    ? await this.stockDataService.getProduct(traders[0])
                    : void 0;
        const rateStr = rate
            ? rate.calculateType === "001"
                ? `${rate.rate * 100}%`
                : rate.calculateType === "002"
                    ? `${rate.rate + rateTarget.productName}`
                    : void 0
            : void 0;
    ...
    async doSubscribe() {
        ...
        if (traderId) {
            ...
            // if(!this._depth$){
            this._depth$ = this.socketioService
            ...
            console.log(
                "trade-interface_realtimeData:reportArr: ",
                this.reportArr,
            );
    ...
    getProcessEntrusts(infiniteScroll?: InfiniteScroll) {
        this.entrustServiceProvider
            .getEntrusts(this.traderId, "001,002")
            .then(data => {
