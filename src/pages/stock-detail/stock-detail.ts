...
export class StockDetailPage {
    ...
    doSubscribe() {
        ...
        if (this._stockCode) {
            ...
            this._saleableQuantity$ = this.personalDataService.personalStockList$
                .map(arr =>
                    arr.filter(item => item.stockCode === this._stockCode),
                )
                .map(arr => (arr.length && +arr[0].saleableQuantity) || 0)
                .distinctUntilChanged();
            this._baseData$ = this.stockDataService.stockBaseData$.map(
                data => data[stockCode],
            );
            this._realtimeData$ = this.stockDataService.stockRealtimeData$.map(
                data => data[stockCode],
            );
            this._kData$ = this.stockDataService.stockKData$.map(
                data => data[stockCode] || {},
            );
    ...
    checkMarket(index, saleableQuantity, baseData) {
        ...
        const buyTempPrice = (this.buyTempPrice =
            buy1Price || sale1Price || latestPrice || 0);
        // 卖出价格同理，在跌停时需要特殊处理。
        const saleTempPrice = (this.saleTempPrice =
            sale1Price || buy1Price || latestPrice || 0);
        ...
        if (index < factorArrayLength) {
            ...
            this.buyTotalQuantity =
                Math.floor(
                    (maxBuyQuantity * this.factorArray[index]) / oneHand,
                ) * oneHand;
            this.saleTotalQuantity =
                Math.floor(saleQuantity / oneHand) * oneHand;
        } else {
            const buyQuantity =
                oneHand * this.convenientHandArray[index - factorArrayLength];
            this.buyTotalQuantity =
                buyQuantity <= maxBuyQuantity ? buyQuantity : 0;
            this.saleTotalQuantity =
                buyQuantity <= saleableQuantity ? buyQuantity : 0;
        }
        ...
        return {
            buyMarket: this.checkRiseOrFall(
                buyTempPrice,
                yesterdayPrice,
                this.buyTotalQuantity,
            ),
            saleMarket: this.checkRiseOrFall(
                saleTempPrice,
                yesterdayPrice,
                this.saleTotalQuantity,
            ),
        ...
        if (this[`${trade}TotalQuantity`] > 0) {
            this.tradeService
                ...
                .then(resData => {
                    ...
                    const result =
                        resData instanceof Array ? resData[0] : resData;
    ...
    showKDataCustomTooltip(data) {
        ...
        const sourceData = this.stockDataService.getStockKData(
            this._stockCode,
            this.kDataUnit,
        );
    ...
    showRealtimeCustomTooltip(data) {
        ...
        const realtimeData = this.stockDataService.getStockRealtimeData(
            this._stockCode,
        );
        const baseData = this.stockDataService.getStockBaseData(
            this._stockCode,
        );
        ...
        const changeValue =
            realPrice - (baseData ? baseData.yesterdayPrice : 0);
