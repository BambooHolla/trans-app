...
export class CommissionListPage implements OnInit {
    ...
    initData() {
        ...
        for (let j = 0; j < this.titleArray.length; j++) {
            ...
            for (let i = 0; i <= dd; i++) {
                const x = i / dd;
                const y = backInOut(x) * (1 + Math.random() * 0.3);
        ...
        this.http
            ...
            .then(response => {
                ...
                if (resData && resData.data) {
                    data = resData.data
                        //过滤撤单委托
                        .filter(item => item.FID_CXBZ != "W")
                        .map(item => ({
                            id: item.FID_WTH,
                            name: item.FID_GQMC,
                            time: `${item.FID_WTRQ.replace(
                                /(\d{4})(\d{2})(\d{2})/,
                                "$1/$2/$3",
                            )} ${item.FID_WTSJ}`,
            ...
            .then(data => {
                ...
                if (!data) {
                    ...
                    (data as any[])
                        ...
                        .map(async item => {
                            let product = await this.stockDataService.getProduct(
                                item.productId,
                            );
                            ...
                            return {
                                ...
                                state:
                                    item.entrustOperationType == "001"
                                        ? "买入"
                                        : item.entrustOperationType == "002"
                                            ? "卖出"
                                            : "", //委托操作类型（001买入、002卖出）
                            };
                        });
    ...
    cancelCommission(commissionId) {
        ...
        const url = `${this.AppSettings.SERVER_URL +
            this.AppSettings
                .SERVER_PREFIX}/transaction/entrusts/cancel/${commissionId}`;
        ...
        this.appService
            ...
            .then(data => {
                ...
                if (!data) {
                    ...
                    (data as any[]).filter(item => item).map(item => ({
                        id: item.id,
                        commit: item.entrustPrice,
                        average: (
                            item.completeTotalPrice /
