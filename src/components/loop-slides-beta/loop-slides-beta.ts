...
export class LoopSlidesBetaComponent {
    ...
    ngAfterViewInit() {
        ...
        this.itemLocationY =
            window
                .getComputedStyle(this.frontCard.nativeElement)
                .getPropertyValue("top") || "0";
        this.itemHeight = window
            .getComputedStyle(this.frontCard.nativeElement)
            .getPropertyValue("height");

        this.boundaryTop =
            parseFloat(this.itemLocationY) - parseFloat(this.itemHeight);
        this.boundaryBottom =
            parseFloat(this.itemLocationY) + parseFloat(this.itemHeight);

        this.transBoundaryTop =
            parseFloat(this.itemLocationY) - parseFloat(this.itemHeight) / 2; // * 2 / 3;
        this.transBoundaryBottom =
            parseFloat(this.itemLocationY) +
            (parseFloat(this.itemHeight) * 2) / 3;
    ...
    touchmoveFun(event: any) {
        ...
        this.backCardInfo =
            spanY > 0
                ? this.originData[
                      (this.index - 1 + this.originData.length) %
        ...
        //限定移动边界
        transitionH =
            transitionH < this.boundaryTop
                ? this.boundaryTop
                : transitionH > this.boundaryBottom
                    ? this.boundaryBottom
                    : transitionH;
    ...
    touchendFun(event: any) {
        ...
        if (transitionH < this.transBoundaryTop) {
            ...
            this.frontCard.nativeElement.setAttribute(
                "style",
                "top:" + this.boundaryTop + "px;opacity:0;transition:.5s",
            );
            ...
            this.frontCard.nativeElement.setAttribute(
                "style",
                "top:" + this.boundaryBottom + "px;opacity:0;transition:.5s",
            );
            this.index =
                (--this.index + this.originData.length) %
                this.originData.length;
        } else {
            // this.frontCard.nativeElement.style = "top:" + this.itemLocationY + ";transition:.5s";
            this.frontCard.nativeElement.setAttribute(
                "style",
                "top:" + this.itemLocationY + ";transition:.5s",
            );
        }
        setTimeout(() => {
