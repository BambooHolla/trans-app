...
function getPageContentRef(page: ViewController) {
    ...
    while (c_ele) {
        if (c_ele.tagName.toUpperCase() === "ION-CONTENT") {
            return (page["_top_ion_content_element_ref"] = new ElementRef(
                c_ele,
            ));
        }
...
export class CommonTransition extends PageTransition {
    init() {
        ...
        const opts = this.opts;
        this.duration(
            opts.duration && isPresent(opts.duration)
                ? opts.duration
                : DURATION,
        );
        this.easing(
            opts.easing && isPresent(opts.easing) ? opts.easing : EASING,
        );
        ...
        if (enteringView) {
            ...
            const enteringPageEle: Element = enteringView.pageRef()
                .nativeElement;
            // entering content
            ...
            enteringContent.element(
                enteringPageEle.querySelectorAll(".animation-header"),
            );
            this.add(enteringContent);
            ...
            if (enteringHasNavbar) {
                ...
                const enteringNavbarItems = new Animation(
                    plt,
                    enteringNavbarEle.querySelectorAll(
                ...
                if (backDirection) {
                    ...
                    if (enteringView.enableBack()) {
                        ...
                        const enteringBackBtnText = new Animation(
                            plt,
                            enteringNavbarEle.querySelector(
                                ".back-button-text",
                            ),
                        );
        ...
        if (leavingView && leavingView.pageRef()) {
            ...
            const leavingContent = new Animation(
                plt,
                getPageContentRef(leavingView),
            );

            //  "ion-header > *:not(ion-navbar),ion-footer > *",
            leavingContent.element(
                leavingPageEle.querySelectorAll(".animation-header"),
            );
            this.add(leavingContent);
            ...
            if (leavingHasNavbar) {
                ...
                const leavingNavbarItems = new Animation(
                    plt,
                    leavingNavbarEle.querySelectorAll(
                        "ion-buttons,[menuToggle]",
                    ),
                );
