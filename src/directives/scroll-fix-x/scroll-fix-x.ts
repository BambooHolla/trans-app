...
export class ScrollFixXDirective {
  ...
  onClick(target: HTMLElement){
    ...
    const scrollContent = scrollElem.querySelector('.scroll-content') as HTMLElement
    // querySelectorAll 的返回值是 NodeList ，不是真正的数组，
    // 需要使用 Array.from() 转换为数组，以便后面使用。
    const items = Array.from(scrollElem.querySelectorAll('.scroll-zoom-wrapper > *')) as HTMLElement[]
    ...
    const index: number = items.findIndex(item =>
      item === target || (item.compareDocumentPosition(target) & 16) === 16
    )
    ...
    const totalWidth = items.reduce((all, item) =>
      all + parseFloat(window.getComputedStyle(item).width), 0
    )
    const prevTotalWidth = activeItem.offsetLeft
    const currentWidth = parseFloat(window.getComputedStyle(activeItem).width)

    const leftSpacing = (contentWidth - currentWidth) / 2
    // 计算 scrollLeft 的目标值，以便激活的元素能尽量显示在水平居中位置，
    // 并且对计算出的值进行有效性限制。
    const destScrollLeft = Math.min(totalWidth - contentWidth, Math.max(0, prevTotalWidth - leftSpacing))
