...
export class CameraModal {
    ...
    initGestureListener() {
        this.pressGesture = new Gesture(
            this.scrollerRef._scrollContent.nativeElement,
        );
            ...
            .subscribe(e => {
                ...
                if (e.eventType === 1) {
                    ...
                    this.imageScale = Math.min(
                        3,
                        Math.max(0.5, e.scale * this.scaleBase),
                    );
                }
            });
    ...
    startCamera() {
        const cameraPreviewOpts: CameraPreviewOptions = {
            ...
            height:
                (this.keyboardService.fullHeight || window.screen.height) - 58,
            camera: "rear",
            tapPhoto: false,
            previewDrag: false,
            toBack: true,
    ...
    getAllTransform() {
        return this.sanitizer.bypassSecurityTrustStyle(
            `translate(${this.translateX}px, ${this.translateY}px) scale(${
                this.imageScale
            })`,
        );
    }
    ...
    recoveryDomStyle(recovery: boolean) {
        ...
        const ngComponent = document.querySelector(
            "ng-component",
        ) as HTMLElement;
        if (recovery) {
