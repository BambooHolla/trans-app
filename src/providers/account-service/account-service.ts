...
export class AccountServiceProvider {
    ...
    static getDealResultDetail(dealResult: DealResult) {
        if (dealResult == DealResult.Preparing)
            return window["language"]["PREPARING"] || "准备中";
        if (dealResult == DealResult.InAudit)
            return window["language"]["VERIFYING"] || "审核中";
        if (dealResult == DealResult.InConfirmation)
            return window["language"]["CONFIRMING"] || "确认中";
        if (dealResult == DealResult.Finish)
            return window["language"]["FINISHED"] || "完成";
        if (dealResult == DealResult.Failed)
            return window["language"]["FAIL"] || "失败";
        if (dealResult == DealResult.Expired)
            return window["language"]["EXPIRE"] || "过期";
        if (dealResult == DealResult.Other)
            return window["language"]["OTHERS"] || "其他";
    }
    static getTransactionStatusDetail(status: TransactionStatus) {
        if (status == TransactionStatus.Unpaied)
            return window["language"]["TO_BE_PAID"] || "待支付";
        if (status == TransactionStatus.Paied)
            return window["language"]["CONFIRMING"] || "确认中"; //'已支付';
        if (status == TransactionStatus.Confirmed)
            return window["language"]["CONFIRMED"] || "已确认";
    ...
    getDeliverType(productIds, type) {
