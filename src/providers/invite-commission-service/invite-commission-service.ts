...
export class InviteCommissionServiceProvider {
    ...
    readonly GET_RECOMMEND_COUNT = this.appSetting.APP_URL(
        "user/getRecommendCount",
    );
    readonly GET_MY_RECOMMENDER = this.appSetting.APP_URL(
        "user/getMyRecommender",
    );
    readonly SET_INVITATION_CODE = this.appSetting.APP_URL(
        "user/setInvitationCode",
    );

    getRecommendCount(customerId: string) {
        return this.fetch.autoCache(true).get(
            this.GET_RECOMMEND_COUNT,
            {
                search: {
    ...
    setInvitationCode(code: string) {
        return this.fetch.autoCache(true).post(
            this.SET_INVITATION_CODE,
            {
                code,
            },
            undefined,
