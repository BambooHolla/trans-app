...
export class AppSettings {
  ...
  public readonly SERVER_URL: string = 'http://192.168.61.235:40001'; //company mac server
  ...
  public readonly LOGIN_URL: string = `${this.SERVER_URL + this.SERVER_PREFIX}/user/login`;
  ...
  public agreementData = {
    riskNote: {
      ...
      '<p class="ti">由于股权交易过程中，可能会面临各种风险因素，<span class="fwb">福建省高交所股权众筹交易中心</span>郑重提示：</p>' +
      ...
      '<div class="mt10 tr">福建省高交所股权众筹交易中心</div>',
    },
    initiationAgreement: {
      title:'投资人员入会协议',
      agreementFirst: '协议编号：____________' +
    ...
    partnershipAgreement: {
      ...
      agreementFirst: '<div class="xy_cont_title">有限合伙投资协议、有限合伙财产份额转让协议<br>《____合伙企业（有限合伙）合伙协议》 <br><br>第一章 总则 </div>' +
  ...
  initMinuteOffset(){
    ...
    switch (this._minuteFixType){
      case MinuteFixType.BeforeTrading:
        ...
        this._minuteOffset = currentMinute - minutePeriods[0].start +
          this.TRADE_DAY_SWITCH_MINUTES_BEFORE_TRADING + 1;
        break;
      case MinuteFixType.BeforeAuction:
        this._minuteOffset = currentMinute - minutePeriods[0].start +
            this.AUCTION_MINUTES_BEFORE_TRADING + 1;
        break;
      case MinuteFixType.Morning:
        // 设定为早上开盘后一小时左右。
        this._minuteOffset = Math.round(currentMinute / 30) * 30 - minutePeriods[0].start - 60;
        break;
      ...
      case MinuteFixType.Afternoon:
        // 设置为下午开盘后一小时左右。
        this._minuteOffset = Math.round(currentMinute / 30) * 30 - minutePeriods[1].start - 60;
        break;
  ...
  initTradeTime() {
    ...
    this.tradingTime.forEach(({ start, end }) => {
      ...
      const endMinutes = dateMoment.hours() * 60 + dateMoment.minutes() - 1 + this.TO_UTC_MINUTES_OFFSET;
      minutePeriods.push({
        start: startMinutes,
        end: endMinutes,
      });
