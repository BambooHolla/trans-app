...
export class PersonalDataService {
    ...
    private _personalStockListIsNull: BehaviorSubject<
        boolean
    > = new BehaviorSubject(undefined);
    personalStockListIsNull$: Observable<
        boolean
    > = this._personalStockListIsNull.asObservable().distinctUntilChanged();
    ...
    requestPersonalData() {
        this.requestCustomerData().catch(err => {});

        this.requestFundData().catch(err => {});

        this.requestEquityDeposit().catch(err => {});

        this.requestCertifiedStatus().catch(err => {});
        ...
        return this.appService
            ...
            .then(data => {
                ...
                if (data.status === "2") {
                    ...
                    this._certifiedMsg =
                        window["language"]["NOT_BEEN_CERTIFIED"] ||
                        data.message;
                } else if (data.status === "1") {
                    this._certifiedMsg =
                        window["language"]["IN_REVIEWING"] || data.message;
                } else if (data.status === "3") {
                    this._certifiedMsg =
                        window["language"]["FAIL_THE_REVIEW"] || data.message;
                } else {
    ...
    parseEquityDeposit(data) {
        ...
        this._personalStockList.next(
            data.map(item => ({
                stockCode: item.FID_GQDM || item.productHouseId,
    ...
    get todayBalance() {
        ...
        return this._personalStockList
            .getValue()
            .reduce((sum, { stockCode, restQuantity }) => {
                ...
                return (
                    ...
                    (baseData && baseData.changeValue
                        ? baseData.changeValue * restQuantity
                        : 0)
                );
            }, 0);
    }
    ...
    requestBankAccount(): Promise<any> {
        ...
        }/api/v1/gjs/biz/accounts/depositoryAccountRelation`;
    ...
    setRelatedBankAccount(data) {
        this._relatedBankAccount.bankName =
            this.bankList[data.FID_YHDM] || "未知银行";
        this._relatedBankAccount.bankIconURL =
            this.bankIconList[data.FID_YHDM] || "";
