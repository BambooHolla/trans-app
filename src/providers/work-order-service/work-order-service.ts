...
export class WorkOrderServiceProvider {
    ...
    static getReplyRoleDetail(role: ReplyRole) {
        if (role == ReplyRole.admin)
            return window["language"]["SUPER_MANAGEMENT"] || "超級管理";
        if (role == ReplyRole.service)
            return window["language"]["CUSTOMER_SERVICES"] || "客服";
        if (role == ReplyRole.customer)
            return window["language"]["CUSTOMER"] || "客戶";
        return status;
    }
    ...
    getWorkOrderList(search?: {
        ...
        return this.fetch
            .autoCache(true)
            .get<ContactModel[]>(this.CONTACTS, { search });
    }
    getContactReplyList(
        contactId: string,
        search?: {
        ...
        return this.fetch
            .autoCache(true)
            .get<ReplyModel[]>(this.CONTACT_REPLY, {
                search,
                params: { contactId },
            });
    }
