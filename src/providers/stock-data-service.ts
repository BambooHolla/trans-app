...
export class StockDataService {
    ...
    private _stockObservableMap: Map<
        string,
        Map<string, { refCounted: Observable<any>; subject: Subject<any> }>
    > = new Map();
    ...
    // 实时数据
    private _stockRealtimeData: BehaviorSubject<
        AnyObject
    > = new BehaviorSubject({});
    ...
    // 因此需要对计算结果做校正。
    numberFixed2: (any) => number = src =>
        Math.round((parseFloat(src) || 0) * 100) / 100;
    ...
    private getTradeTimeMisc() {
        const switchMinutes =
            this.appSettings.tradingMinutePeriods[0].start -
    ...
    private initTradeDay() {
        const {
            ...
            dayOfWeek,
            todayIsTradeDay,
        } = this.getTradeTimeMisc();

        const tempDateMoment = moment.utc(
            dateMoment.format("YYYY-MM-DD 00:00:00.000"),
        );
        ...
        const {
            tradingMinutePeriods,
            TRADE_DAY_SWITCH_MINUTES_BEFORE_TRADING,
        } = this.appSettings;
        this._thisTradeDayEndTime =
            this._thisTradeDayStartTime +
            60e3 *
                (tradingMinutePeriods[1].end -
                    tradingMinutePeriods[0].start +
                    TRADE_DAY_SWITCH_MINUTES_BEFORE_TRADING);
    }
    ...
    private startTradeDayCheckTimer() {
        ...
        this._tradeDayCheckTimer = setInterval(
            this.checkTradeDay.bind(this),
            30e3,
        );
    }
    ...
    private _stockKData: BehaviorSubject<{
     ...
     */
    private checkEndOfPeriodCommon(
        timeUnit: string,
        data: any[],
        index: number,
    ): number {
     ...
     */
    private checkEndOfPeriodOfWeek: (
        data: any[],
        index: number,
    ) => number = this.checkEndOfPeriodCommon.bind(this, "week");
     ...
     */
    private checkEndOfPeriodOfMonth: (
        data: any[],
        index: number,
    ) => number = this.checkEndOfPeriodCommon.bind(this, "month");
    ...
    subscibeRealtimeData(
        ...
        if (!observableMap.has(code)) {
            ...
            if (this.appSettings.SIM_DATA) {
                source$ = this.simSubscribeEquity(
                    code,
                    this.SOCKETIO_SUBSCRIBE_STOCK_EVENT,
                );
            } else {
                // //gjs
            ...
            const equity$ = source$.do(result => {
                console.log("data changed: " + eventName, result);

                const transformedData = this.transformRealtimeData(
                    code,
                    result,
                );
                this.parseAndSetRealtimeData(
                    code,
                    [transformedData],
                    this._stockRealtimeData.getValue()[code],
                );
                ...
                if (true) {
                    ...
                    // 是否需要检查集合竞价已完成。
                    const needCheckAuctionDone =
                        this.appSettings.SIM_DATA &&
                        minute >=
                            -this.appSettings
                                .AUCTION_DONE_MINUTES_BEFORE_TRADING;
                    // 若 baseData 已存在，并且 auctionDone 属性值为 false ，
                    // 则需要将其设为 true ，并采用当前模拟出的最新价格作为开盘价。
                    const toAuctionDone =
                        needCheckAuctionDone &&
                        baseData &&
                        !baseData.auctionDone;
    ...
    private simStockBaseData(code) {
        this.parseStockBaseData(
            code,
            this._baseDataForSim[code] || this._baseDataForSim.default,
        );
    }
    ...
    private setStockBaseData(code, subData) {
        ...
        if (turnoverQuantity) {
            const minPrice = startPrice
                ? Math.min(startPrice, latestPrice)
                : latestPrice;
            if (!baseData.minPrice || minPrice < baseData.minPrice) {
                extraData.minPrice = minPrice;
            }
            const maxPrice = startPrice
                ? Math.max(startPrice, latestPrice)
                : latestPrice;
        ...
        // 属性无效时，需要将其删除。
        if (
            typeof turnoverQuantity === "string" &&
            /^[+-]/.test(turnoverQuantity)
        ) {
            extraData.turnoverQuantity =
                baseData.turnoverQuantity + parseInt(turnoverQuantity, 10);
            extraData.turnoverAmount = this.numberFixed2(
                baseData.turnoverAmount + parseFloat(turnoverAmount),
            );
    ...
    private refreshDataForSim(code, extraData) {
        ...
        if ("turnoverQuantity" in extraData) {
            dataForSim.FID_CJSL = extraData.turnoverQuantity;
        }
        if ("turnoverAmount" in extraData) {
            dataForSim.FID_CJJE = extraData.turnoverAmount;
        }
    }
    ...
    private parseKData(code: string, unit: string, data: any[]) {
        ...
        data = data.filter(
            element => moment(element.FID_RQ).isValid() && element.FID_CJSL > 0,
        );
        ...
        while (startIndex < data.length) {
            ...
            const date = subData[subDataLength - 1].FID_RQ.replace(
                /^(\d{4})(\d{2})(\d{2})/,
                "$1-$2-$3",
            );
            ...
            subData.forEach(element => {
                minPriceOfPeriod = Math.min(
                    minPriceOfPeriod,
                    parseFloat(element.FID_ZDJ),
                );
                maxPriceOfPeriod = Math.max(
                    maxPriceOfPeriod,
                    parseFloat(element.FID_ZGJ),
                );
    ...
    parseMinute(dateString: string): number | boolean {
        ...
        const {
            tradingMinutePeriods,
            AUCTION_MINUTES_BEFORE_TRADING,
        } = this.appSettings;
        ...
        if (
            minute >=
                tradingMinutePeriods[0].start -
                    AUCTION_MINUTES_BEFORE_TRADING &&
            minute <= tradingMinutePeriods[0].end
        ) {
            // 当前分钟落在上午交易时间段内，则减去上午开盘时间的分钟数。
            return minute - tradingMinutePeriods[0].start;
        } else if (
            minute >= tradingMinutePeriods[1].start &&
            minute <= tradingMinutePeriods[1].end
        ) {
            ...
            return (
                minute -
                tradingMinutePeriods[1].start +
                tradingMinutePeriods[0].end -
                tradingMinutePeriods[0].start +
                1
            );
        } else {
    ...
    private fillEmptyData(code, dataArray, minute) {
        ...
        if (
            ...
            const fillData = {
                price:
                    prevMinute >= 0
                        ? dataArray[prevMinute].price
                        : baseData.yesterdayPrice,
                turnoverQuantity: 0,
            };
    ...
    private getSimStartData(code) {
        ...
        const useLastTradeDay =
            !todayIsTradeDay || currentMinutes < switchMinutes;
        if (useLastTradeDay) {
            const lastTradeDayOffset = dayOfWeek > 1 ? 1 : 2 + dayOfWeek;
        ...
        const dayString = moment
            .utc(+dateMoment - minuteOffset * 60e3)
            .format("YYYY-MM-DD");
        ...
        const detailEndIndex = Math.min(
            Math.floor(second / 5) + 1,
            lastDetailData.length,
        );
        let totalQuantityOfMinute = 0;
        let totalAmountOfMinute = 0;
        for (let i = 0; i < detailEndIndex; i++) {
            const { price, turnoverQuantity, turnoverAmount } = lastDetailData[
                i
            ];
            lastEndPrice = price;
    ...
    private simRealtimeData(code, dayString) {
        ...
        const baseData = this._stockBaseData.getValue()[code];
        const yesterdayPrice = baseData
            ? baseData.yesterdayPrice
            : this._baseDataForSim[code].FID_ZSP;
        const handBase = baseData
            ? baseData.handBase
            : this._baseDataForSim[code].FID_JYJS;
        ...
        const randomData = () => {
            ...
            const minutes =
                dateMoment.get("hour") * 60 + dateMoment.get("minute");
            ...
            const now = Math.min(
                ...
                Math.max(
                    ...
                    this.numberFixed2(
                        seemNotChange
                            ? lastEndPrice +
                              (Math.random() > 0.6 ? sign * 0.01 : 0)
                            : lastEndPrice *
                              (1 + Math.pow(Math.random(), 2) * 0.02 * sign),
                    ),
                ),
            );

            // turnoverQuantity 的单位设为“股”。
            const turnoverQuantity =
                Math.floor(Math.random() * 80 + 2) * handBase;
            const turnoverAmount = this.numberFixed2(
                now * turnoverQuantity * (0.97 + Math.random() * 0.06),
            );
            fullStartData.push({
                time,
                minutes,
        ...
        for (
            dateMoment.add(start_1, "minutes");
            +dateMoment <= midTime;
            dateMoment.add(5, "seconds")
        ) {
            randomData();
        }

        for (
            dateMoment.add(interTrade * 60 - 5, "seconds");
            +dateMoment <= endTime;
            dateMoment.add(5, "seconds")
        ) {
            randomData();
        ...
        for (let i = 0; i <= fullStartData.length; i++) {
            const { time, minutes, now, turnoverQuantity, turnoverAmount } =
                fullStartData[i] || ({ minutes: 1e5 } as any);
    ...
    private simSubscribeEquity(
        ...
        const calculatePriceBound = () => {
            ...
            const yesterdayPrice = baseData
                ? baseData.yesterdayPrice
                : this._baseDataForSim[equityCode].FID_ZSP;
        ...
        const observable = Observable.timer(0, 5e3)
            ...
            .map(() => {
                ...
                // 对 dateMoment 进行时间修正后再取日期字符串。
                const dayString = moment
                    .utc(+dateMoment - this.appSettings.minuteOffset * 60e3)
                    .format("YYYY-MM-DD");
                const realtimeSimData = this.realtimeSimData;
                if (
                    !realtimeSimData[equityCode] ||
                    !realtimeSimData[equityCode][dayString]
                ) {
                    return;
                }
                ...
                if (currentMinutes > end_2 || currentMinutes < start_1) {
                    ...
                    minuteIndex =
                        currentMinutes - start_2 + 1 + end_1 - start_1;
                }

                if (
                    realtimeData.length <= minuteIndex ||
                    realtimeData[minuteIndex].detail.length <= detailIndex
                ) {
                    return;
                }
                ...
                const {
                    ...
                    turnoverQuantity: tq,
                    turnoverAmount: ta,
                } = realtimeData[minuteIndex].detail[detailIndex];
                ...
                for (let i = 5; i > -5; i--) {
                    ...
                    // 与实时价格相同的价位被锁在 买一 档。
                    bets.push(
                        betPrice > maxPriceBound || betPrice < minPriceBound
                            ? {
                                  price: 0,
                                  quantity: 0,
                              }
    ...
    private randomKData(
        ...
        while ((dateString = dateMoment.format("YYYYMMDD")) <= endDate) {
            ...
            const turnoverAmount =
                turnoverQuantity * (minPrice + maxPrice) * 50;
            data.push({
                FID_RQ: dateString,
            ...
            data.map(product => {
                ...
                if (product.productType == "001") {
                    this.appDataService.products.set(product.productHouseId, {
                        ...
                        expire:
                            new Date().getTime() +
                            this.appSettings.EXPIRE_TIME_SPAN,
                    });
                }
            }),
        );
    ...
    public async getProduct(productId) {
        ...
        if (
            ...
            (product.expire && product.expire < now)
        ) {
